...
| 🎛️ **8 种可调效果** | 颗粒感、色彩分级、暗角、漏光、褪色、光晕、光晕发光、色调曲线 || 🔏 **文字水印** | 自定义文字水印烧录至导出图片 — 支持多行、字形、字重、字体、位置、不透明度 || 🖼️ **前后对比** | 可拖动分割线，随时对比调整前后效果 |
...
| **光晕** | 强度 · 颜色（红/暖/金） · 半径 |
| **发光** | 强度 · 阈值 · 半径 |
| **色调曲线** | 阴影 · 中间调 · 高光（五点样条曲线） || **水印** | 启用 · 文字（支持多行） · 位置 · 颜色 · 不透明度 · 大小 · 字重/字形/字体 |
...
在**调整**面板底部找到 **水印** 区块并展开。

1. 打开**启用**开关。
2. 输入**文字** — 支持多行（按 Enter 换行）。
3. 设置**位置**：左上 / 右上 / 左下 / 右下 / 居中。
4. 选择**颜色**（白色或黑色）、**不透明度**（10–100%）、**大小**（10–100）。
5. 调整**字体**：字重（加粗/常规） · 字形（正常/斜体） · 字体（无袅线体/袅线体/等宽体）。

水印会实时在画布预览中显示，导出时（包括单张和批量）永久烧录到图片中。

水印设置独立于胶片预设存储，切换预设不会重置水印。
